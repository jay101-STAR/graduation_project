...
module eeprom (
  ...
  task write_to_eeprm;
    begin
      // Loop to handle multiple byte writes with address auto-increment
      forever begin
        shift_in(memory_buf);
        address         = {ctrl_byte[3:1], addr_byte};
        memory[address] = memory_buf;
        $display("eeprm---memory[%0h]=%0h", address, memory[address]);
        // Auto-increment address for next byte
        addr_byte = addr_byte + 1;
        // Check if STOP condition is coming (detected by negedge sda in main block)
        // If STOP detected, disable will be called to exit this loop
      end
    end
  endtask
    ...
    begin
      ...
      if(ctrl_byte == r7 || ctrl_byte == r6 || ctrl_byte == r5 || ctrl_byte == r4 || ctrl_byte == r3 || ctrl_byte == r2 || ctrl_byte == r1 || ctrl_byte == r0)begin
        ...
        forever begin
          ...
          // After shift_out, wait for ACK/NACK bit (9th clock cycle)
          @(posedge scl);  // ACK/NACK bit (scl high, read sda)
          if (sda == 1) begin  // NACK received
            $display("eeprm---NACK received, stop reading");
            @(posedge scl);  // Wait for ACK/NACK cycle to complete
            break;  // Stop reading
          end else begin
            // ACK received, increment address for next byte
            $display("eeprm---ACK received, increment address to next byte");
            addr_byte = addr_byte + 1;
            @(posedge scl);  // Wait for ACK/NACK cycle to complete
          end
        end
