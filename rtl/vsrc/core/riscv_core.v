...
module riscv_core (
    input clk,
    input rst,
    input top_core_mem_stall,

    input  [31:0] instrom_core_data,
    input  [31:0] instrom_core_pc,
    output [31:0] core_instrom_addr,
    output        core_instrom_ren,
    output [31:0] tohost_value_register,

    output [31:0] core_dataram_addr,
    output [31:0] core_dataram_wdata,
    output        core_dataram_wen,
    output        core_dataram_ren,
    output [ 7:0] core_dataram_alucex,
    output        core_dataram_stall,
    output reg [31:0] core_bp_branch_total,
    output reg [31:0] core_bp_mispredict_total,
    output reg [31:0] core_bp_target_miss_total,
    input  [31:0] dataram_core_rdata
);
  ...
  // Control signals
  assign stall_if_id = load_use_hazard || mul_hazard || div_hazard || top_core_mem_stall;
  assign stall_id_ex = mul_hazard || div_hazard || top_core_mem_stall;
  assign stall_ex_dataram = mul_hazard || div_hazard || top_core_mem_stall;  // Also stall EX/MEM register during multiplication/division
  assign stall_dataram_wb = stall_ex_dataram;
  ...
  assign core_dataram_addr = dataram_addr;
  assign core_dataram_wdata = dataram_wdata;
  assign core_dataram_wen = dataram_wen;
  assign core_dataram_ren = dataram_ren;
  assign core_dataram_alucex = dataram_alucex;
  assign core_dataram_stall = stall_ex_dataram;
  // Retire one instruction when MEM->WB advances with a valid entry.
  assign retire_pulse = (!stall_dataram_wb) && ex_dataram_valid;

  // Branch prediction statistics counters
  always @(posedge clk) begin
    if (rst) begin
      core_bp_branch_total <= 32'b0;
      core_bp_mispredict_total <= 32'b0;
      core_bp_target_miss_total <= 32'b0;
    end else begin
      if (ex_bp_event_branch) begin
        core_bp_branch_total <= core_bp_branch_total + 32'b1;
      end
      if (ex_bp_event_mispredict) begin
        core_bp_mispredict_total <= core_bp_mispredict_total + 32'b1;
      end
      if (ex_bp_event_target_miss) begin
        core_bp_target_miss_total <= core_bp_target_miss_total + 32'b1;
      end
    end
  end

  // ========== IF Stage: PC Module ==========
  assign if_instruction = instrom_core_data;

  pc pc0 (
      .clk          (clk),
      .rst          (rst),
      .stall        (load_use_hazard || mul_hazard || div_hazard || top_core_mem_stall),
      .ren          (core_instrom_ren),
      ...
      .next_pc      (core_instrom_addr),
      .pc_id_pc     (pc_if_pc)
  );

  // IF1/IF2 front-end pipeline for synchronous BRAM instruction fetch.
  always @(posedge clk) begin
    if (rst) begin
      if1_pc                  <= `PC_BASE_ADDR;
      if1_valid               <= 1'b0;
      ...
      if1_predicted_pc        <= if1_predicted_pc;
      if1_predicted_pht_index <= if1_predicted_pht_index;
      if2_pc                  <= if2_pc;
      if2_instruction         <= if2_instruction;
      if2_valid               <= if2_valid;
      if2_predicted_taken     <= if2_predicted_taken;
      if2_predicted_pc        <= if2_predicted_pc;
      if2_predicted_pht_index <= if2_predicted_pht_index;
      // Capture at most one returned BRAM beat while stalled.
      // Do not overwrite pending data, otherwise we may drop the oldest
      // post-stall instruction (e.g. li before branch in rv32um tests).
      if (!if_stall_pending_valid) begin
        if_stall_pending_pc          <= instrom_core_pc;
        if_stall_pending_instruction <= if_instruction;
        if_stall_pending_valid       <= if1_valid;
        if_stall_pending_predicted_taken <= if1_predicted_taken;
        if_stall_pending_predicted_pc    <= if1_predicted_pc;
        if_stall_pending_predicted_pht_index <= if1_predicted_pht_index;
      end else begin
        if_stall_pending_pc          <= if_stall_pending_pc;
        if_stall_pending_instruction <= if_stall_pending_instruction;
        if_stall_pending_valid       <= if_stall_pending_valid;
        if_stall_pending_predicted_taken <= if_stall_pending_predicted_taken;
        if_stall_pending_predicted_pc    <= if_stall_pending_predicted_pc;
        if_stall_pending_predicted_pht_index <= if_stall_pending_predicted_pht_index;
      end
    end else begin
      // IF1 -> IF2
      if2_pc                  <= if_stall_pending_valid ? if_stall_pending_pc : if1_pc;
      if2_instruction         <= if_stall_pending_valid ? if_stall_pending_instruction : if_instruction;
      if2_valid               <= if_stall_pending_valid ? 1'b1 : if1_valid;
      if2_predicted_taken     <= if_stall_pending_valid ? if_stall_pending_predicted_taken : if1_predicted_taken;
      if2_predicted_pc        <= if_stall_pending_valid ? if_stall_pending_predicted_pc : if1_predicted_pc;
      if2_predicted_pht_index <= if_stall_pending_valid ? if_stall_pending_predicted_pht_index : if1_predicted_pht_index;

      // New IF1 request metadata
      if1_pc                  <= core_instrom_addr;
      if1_valid               <= core_instrom_ren;
      if1_predicted_taken     <= if_bp_predict_taken;
      if1_predicted_pc        <= if_bp_predict_pc;
      if1_predicted_pht_index <= if_bp_predict_pht_index;
      if_stall_pending_valid       <= 1'b0;
    end
  end

  // Only allow valid BRAM return beats into IF/ID.
  ...
  if_id_reg if_id_reg0 (
      ...
      .flush             (flush_if_id),
      .if_pc             (if2_pc),
      .if_instruction    (if2_instruction_aligned),
      .if_valid          (if2_valid),
      .if_predicted_taken(if2_predicted_taken_aligned),
      .if_predicted_pc   (if2_predicted_pc_aligned),
      .if_predicted_pht_index(if2_predicted_pht_index_aligned),
      .id_pc             (id_pc),
  ...
  // MEM->EX forwarding data:
  // - Load: forward memory read data
  // - Others: forward ALU result
  wire [31:0] dataram_forward_data = (ex_dataram_aluc == `L_TYPE) ? dataram_core_rdata :
                                     ex_dataram_alu_result;

  wire [31:0] ex_forwarded_rs1_data = forward_dataram2ex_rs1 ? dataram_forward_data :
                                      forward_wb2ex_rs1 ? wb_reg_rd_data :
                                      ex_rs1_data;
  wire [31:0] ex_forwarded_rs2_data = forward_dataram2ex_rs2 ? dataram_forward_data :
  ...
  ex_dataram_reg ex_dataram_reg0 (
      ...
      .dataram_valid     (ex_dataram_valid),
  ...
  wb wb0 (
      ...
      .dataram_wb_rdata(dataram_core_rdata),
