...
module dataram (
    ...
    output reg [31:0] dataram_ex_rdata
);

  reg [31:0] mem[0:8191];  // 8192 words = 32768 bytes
  ...
  initial begin
    ...
    $readmemh("/home/jay/Desktop/graduation_project/rtl/vsrc/dataram/dataram.hex", mem, 0);
  end

  wire [31:0] addr_offset = ex_dataram_addr - `PC_BASE_ADDR;
  wire [12:0] word_addr = addr_offset[14:2];
  ...
  always @(*) begin
    if (ex_dataram_ren) begin
      case (ex_dataram_alucex)
        `LB_TYPE:  dataram_ex_rdata = {{24{byte_data[7]}}, byte_data};
        `LH_TYPE:  dataram_ex_rdata = {{16{halfword_unaligned[15]}}, halfword_unaligned};
        `LW_TYPE:  dataram_ex_rdata = word_unaligned;
        `LBU_TYPE: dataram_ex_rdata = {24'b0, byte_data};
        `LHU_TYPE: dataram_ex_rdata = {16'b0, halfword_unaligned};
        default:   dataram_ex_rdata = 32'b0;
      endcase
    end else begin
      dataram_ex_rdata = 32'b0;
    end
  end
